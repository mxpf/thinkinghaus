...
const STUDIO_ORIGIN = "https://thinkinghaus-studio.maxpfennighaus.workers.dev";
const ALLOWED_EMAIL = "maxpfennighaus@gmail.com";
...
async function readStudioLibrary(env: Env): Promise<StudioLibrary> {
	const response = await env.STUDIO.fetch(new Request(`${STUDIO_ORIGIN}/api/posts`));
	if (!response.ok) {
...
async function saveToStudio(env: Env, post: StudioPost): Promise<StudioPost> {
	const response = await env.STUDIO.fetch(
		new Request(`${STUDIO_ORIGIN}/api/save`, {
			method: "POST",
...
export class ThinkinghausMCP extends McpAgent<Env, Record<string, never>, Props> {
	...
	async init() {
		if (this.props?.email.toLowerCase() !== ALLOWED_EMAIL) return;

		this.server.tool(
			"save_thinkinghaus_draft",
			"Save a complete new essay, fragment, or revision into the private Thinkinghaus Studio library. This never publishes or deletes anything. For a revision, provide the current title or slug so the existing piece is updated instead of duplicated. Send the complete body, not a patch or excerpt.",
			{
						...
						content: [
							{
								type: "text",
								text: JSON.stringify({
									message: `Saved “${saved.title}” to Thinkinghaus Studio. It has not been published.`,
									title: saved.title,
									slug: saved.slug,
									status: saved.status,
								}),
...
export default new OAuthProvider({
	apiHandler: ThinkinghausMCP.serve("/mcp"),
	apiRoute: "/mcp",
